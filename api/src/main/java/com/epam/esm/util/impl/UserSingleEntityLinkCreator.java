...
public class UserSingleEntityLinkCreator extends SingleEntityLinkCreator {
    ...
    public List<Link> createLinks(UserDto user) throws CustomException {
        Link selfLink = linkTo(methodOn(UserController.class).findUser("")).withSelfRel();
        Link ordersLink = linkTo(methodOn(UserController.class).findOrders("", 1, 10))
                .withRel(ORDERS);
    ...
    public List<Link> createLinks(OrderDto order, Long userId) throws CustomException {
        Link selfLink = linkTo(methodOn(UserController.class).findOrder("", order.getId())).withSelfRel();
        Link ordersLink = linkTo(methodOn(UserController.class).findOrders("", 1, 10))
                .withRel(ORDERS);
        Link createOrderLink = linkTo(methodOn(UserController.class).createOrder("", null))
                .withRel(CREATE_ORDER);
