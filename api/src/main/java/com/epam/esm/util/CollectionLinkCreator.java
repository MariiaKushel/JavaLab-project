...
public abstract class CollectionLinkCreator implements LinkCreator {
    ...
    public List<Link> createLinks(List<TagDto> tags,
        ...
        Link firstPageLink = linkTo(methodOn(TagController.class).findAllTags("", 1, size))
                .withRel(FIRST_PAGE);
        ...
        Link currentPageLink = linkTo(methodOn(TagController.class).findAllTags("", page, size))
        ...
        Link lastPageLink = linkTo(methodOn(TagController.class).findAllTags("", lastPage, size))
                .withRel(LAST_PAGE);
    ...
    public List<Link> createLinksCertificates(List<CertificateDto> certificates,
        ...
        List<Link> links = new ArrayList<>();
        Link firstPageLink = linkTo(methodOn(CertificateController.class).findAllCertificates("", 1, size))
                .withRel(FIRST_PAGE);
        links.add(firstPageLink);
        if (page > 1 && page <= lastPage) {
        ...
        Link lastPageLink = linkTo(methodOn(CertificateController.class).findAllCertificates("", lastPage, size))
                .withRel(LAST_PAGE);
        links.add(lastPageLink);
    ...
    public List<Link> createLinksCertificates(List<CertificateDto> certificates,
        ...
        Link firstPageLink = linkTo(methodOn(CertificateController.class)
                .findAllCertificatesByTags("", 1, size, tags))
                .withRel(FIRST_PAGE);
        links.add(firstPageLink);
        if (page > 1 && page <= lastPage) {
    ...
    public List<Link> createLinksCertificates(List<CertificateDto> certificates,
        ...
        List<Link> links = new ArrayList<>();
        Link firstPageLink = linkTo(methodOn(CertificateController.class)
                .findAllCertificatesByParameters("", 1, size, tag, name, description, sortBy))
                .withRel(FIRST_PAGE);
        links.add(firstPageLink);
        if (page > 1 && page <= lastPage) {
            Link previousPageLink = linkTo(methodOn(CertificateController.class)
        ...
        Link lastPageLink = linkTo(methodOn(CertificateController.class)
                .findAllCertificatesByParameters("", lastPage, size, tag, name, description, sortBy))
                .withRel(LAST_PAGE);
        links.add(lastPageLink);
        Link findAllLink = linkTo(methodOn(CertificateController.class)
                .findAllCertificates("", 1, 10))
